<?php  
	...
	// get Order  detail
	$ordconditions = array ( 
							  'select' 		=> 'invoice_number, donor, invoice_id, id_donation'
							, 'where' 		=> array ( 
    ...
    $roworder = $dblms->getRows(PAYFAST_DONATIONS, $ordconditions);
	...
	if($orderStatus == 'PAID') {  
		...
		if($sqllmsTransInsert){
			...
			$sqllmsUpdate  = $dblms->Update(DONATIONS , $dataInvPayment , " WHERE id = '".cleanvars($roworder['id_donation'])."'");
